...
describe("Processor", () => {
  ...
  describe("[text]", () => {
    it("does nothing if 'text' property is not set", async () => {
      const workflow = getDefaultWorkflow({
        layers: [{ content: { type: "text" } }],
      });

      const image = await processWorkflow(workflow);
      const filename = "text-content-text-not-set";
      await saveArtifact(image, filename);

      expect(await diffPercentage(filename)).toBe(0);
    });

    it("renders with default font, colour and size when text is provided", async () => {
      const workflow = getDefaultWorkflow({
        layers: [{ content: { type: "text", text: "Hello" } }],
      });

      const image = await processWorkflow(workflow);
      const filename = "text-content-basic";
      await saveArtifact(image, filename);
